...
export class ContextCompressor {
  ...
  private static detectChunkType(lowerText: string, text: string): ChunkScore['type'] {
    ...
    if (lowerText.match(/^(answer|solution|result|response):/i)) return 'answer';
  ...
  private static getTypeScore(type: ChunkScore['type'], lowerText: string): number {
    ...
    // Boost score for content with important context indicators
    if (lowerText.includes('important') || lowerText.includes('critical')) {
      score += 10;
    }
    if (lowerText.includes('summary') || lowerText.includes('conclusion')) {
      score += 5;
    }
