...
export class MemoryManager {
  ...
  private listByCategoryStmt: Database.Statement | null = null;
  private searchStmt: Database.Statement | null = null;
  private setPriorityStmt: Database.Statement | null = null;
  private getByPriorityStmt: Database.Statement | null = null;
  ...
  private initializePreparedStatements(): void {
    ...
    // Pre-compile all frequently used statements
    this.saveStmt = this.db.prepare(`
      INSERT OR REPLACE INTO memories (key, value, category, timestamp, lastAccessed, priority)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    
    this.deleteStmt = this.db.prepare(`DELETE FROM memories WHERE key = ?`);
    
    this.updateStmt = this.db.prepare(`
      UPDATE memories SET value = ?, timestamp = ?, lastAccessed = ? WHERE key = ?
    `);
    ...
    this.listByCategoryStmt = this.db.prepare(`
      SELECT * FROM memories WHERE category = ? ORDER BY priority DESC, timestamp DESC
    `);
    ...
    this.setPriorityStmt = this.db.prepare(`UPDATE memories SET priority = ? WHERE key = ?`);
    
    this.getByPriorityStmt = this.db.prepare(`
      SELECT * FROM memories WHERE priority = ? ORDER BY timestamp DESC
    `);
  ...
  public update(key: string, value: string): boolean {
    const timestamp = new Date().toISOString();
    const result = this.updateStmt!.run(value, timestamp, timestamp, key);
    return result.changes > 0;
  }
  ...
  public list(category?: string): MemoryItem[] {
    if (category) {
      return this.listByCategoryStmt!.all(category) as MemoryItem[];
    }
    return this.listAllStmt!.all() as MemoryItem[];
  }
  ...
  public search(query: string): MemoryItem[] {
    const pattern = `%${query}%`;
    return this.searchStmt!.all(pattern, pattern) as MemoryItem[];
  }
  ...
  public getByPriority(priority: number): MemoryItem[] {
    return this.getByPriorityStmt!.all(priority) as MemoryItem[];
  }
  ...
  public setPriority(key: string, priority: number): boolean {
    const result = this.setPriorityStmt!.run(priority, key);
    return result.changes > 0;
  }
  ...
  public getStats(): { total: number; byCategory: Record<string, number> } {
    const categories = this.statsStmt!.all() as Array<{ category: string; count: number }>;
