...
export const monitorConsoleLogsDefinition: ToolDefinition = {
  ...
  description: 'console log|check errors|show logs|console|check logs|debug output|console errors - Monitor browser console',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function monitorConsoleLogs(args: { url: string; logLevel?: string; duration?: number }): Promise<ToolResult> {
  ...
  try {
    ...
    page.on('pageerror', (err: unknown) => {
      ...
      const errorSource = err instanceof Error ? err.stack?.split('\n')[0] || 'unknown' : 'unknown';
